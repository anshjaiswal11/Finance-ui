...
import { getCategoryBreakdown, getMonthlyData, CATEGORIES } from '../../data/mockData';
...
export default function InsightCards() {
  ...
  const dayRange = dates.length
    ? Math.max(1, Math.ceil((Math.max(...dates) - Math.min(...dates)) / (1000 * 60 * 60 * 24)) + 1)
    : 1;
  ...
  const cards = [
    {
      ...
      sub: topCategory ? `${fmt(topCategory.value)} total spent` : 'No data',
  ...
  return (
    <div className="insights-grid">
      {cards.map((card, i) => (
        <div key={i} className="insight-card slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
